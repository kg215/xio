...
const ContextMenu = connect(void 0, (dispatch: any) => {
  return {
    editArticleGroup: (payload: { id: number; title: string }) =>
      dispatch({ type: "article/editArticleGroup", payload }),
    deleteArticleGroup: (id: number) =>
      dispatch({ type: "article/deleteArticleGroup", id }),
    deleteArticle: (id: number) =>
      dispatch({ type: "article/deleteArticle", id }),
  };
})(function ({
  id,
  deleteArticle,
  ...
  return (
    <Dropdown
      overlay={
        <Menu>
          {isGroup && (
            <Menu.Item onClick={() => addArticle(id)}>添加文章</Menu.Item>
          )}
          {isGroup && (
            <Menu.Item onClick={() => addGroup()}>添加分组</Menu.Item>
          )}
    ...
    >
      <div
        onMouseMove={() => setShowDel(true)}
        onMouseLeave={() => {
          setShowDel(false);
        }}
        className="i-tree-node"
      >
        {isGroup && (
          <div
        ...
        {!isGroup && <div className="context-menu__title">{val}</div>}
        <div
          onClick={del}
          className={className({
...
function Write({
  ...
  useEffect(() => {
    let editor = new Editor(
      toolbar.current as HTMLElement,
      text.current as HTMLElement
    );
  ...
  const pullArticle = useCallback((list: any[] = []) => {
    return list.map((item) => {
      return (
        <TreeNode
          switcherIcon={<Icon type="file" />}
          title={<ContextMenu id={item.id} title={item.title} type="article" />}
          key={item.group_id + "-article-" + item.id}
        />
      );
    });
  }, []);
  const loopGroup = useCallback(
    (list: articleGroupList[] = []) => {
          ...
          <TreeNode
            ...
            title={
              <ContextMenu
                ...
                addGroup={() => contextMenuAddGroup(item.id)}
                title={item.title}
              />
            }
            key={String(item.id)}
          >
            {articleGroup.list[item.id] &&
              loopGroup(articleGroup.list[item.id])}
  ...
  const loopGroupSelector = useCallback(
    (list: articleGroupList[] = []) => {
      return Object.values(list).map((item: articleGroupList) => {
        return (
          <TreeNode value={item.id} title={item.title} key={String(item.id)}>
            {articleGroup.list[item.id] &&
              loopGroupSelector(articleGroup.list[item.id])}
          </TreeNode>
        );
      });
    },
      ...
      <div className="main">
        ...
        <div className="titles">
          ...
          <div className="tree">
                ...
                if (node) {
                  ...
                  if (key.includes("-article-")) {
                    let infoIds = key.split("-article-"),
                      ...
                      info = articles["list"][group_id].find(
                        (item: any) => item.id == id
                      ),
            ...
            >
              <TreeNode
                switcherIcon={<Icon type="read" />}
                title={
                  <Dropdown
                    overlay={
                      <Menu>
                        <Menu.Item onClick={() => addArticle()}>
                          添加文章
                        </Menu.Item>
                        <Menu.Item onClick={() => contextMenuAddGroup()}>
                          添加分组
                        </Menu.Item>
                      </Menu>
                    }
                    ...
                    <div className="i-tree-node">
                      ...
                      <div ref={addGroupRef}>
                        <Popconfirm
                          ...
                          icon={
                            <Icon type="cluster" style={{ fontSize: 24 }} />
                          }
                          onConfirm={() => addGroup()}
                          onCancel={() => setGroupTitle("")}
                          cancelText="取消"
